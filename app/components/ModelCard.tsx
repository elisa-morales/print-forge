...
export default function ModelCard({ model }: ModelCardProps) {
  return (
    <Link href={`/3d-models/${model.id}`} className='block group hover:shadow-[0_5px_12px_rgba(0,0,0,0.1)] hover:-translate-y-[3px] transition-all' aria-labelledby={`model-${model.id}-title`}>
      <div className='overflow-hidden transition-shadow bg-white rounded-lg shadow-md hover:shadow-lg' role='article'>
        ...
        <div className='p-4'>
          ...
          <p className='text-gray-800 text-sm line-clamp-2 min-h-[2.5rem] leading-[1.25rem]'>{model.description}</p>
          <div className='mt-2'>
